...
from app.providers.thefork.mapper import (
    to_availability_result,
    to_contract_reservation,
)


def test_maps_thefork_reservation_payload_to_contract_reservation():
    payload = {
        "id": "tf-res-123",
        "status": "confirmed",
        "party_size": 4,
    ...
    assert reservation.ref.external_id == "tf-res-123"
...
def test_maps_thefork_availability_payload_to_contract_result():
    payload = {
        "slots": [
            {
                "start": "2026-07-01T19:30:00Z",
    ...
    assert str(slot.slot_token) == "tf-slot-token-123"
